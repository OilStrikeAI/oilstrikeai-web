...
## "amount" is always the single-occurrence figure — hard rule, for stability across re-analysis
"amount" must always be the per-occurrence figure: the dollar value of ONE shipment, ONE cycle, ONE instance of the finding — never a total pre-multiplied across multiple shipments, months, or years. If a $0.50/barrel discrepancy applies to a 500,000-barrel monthly cargo, "amount" is $250,000 (that one cargo's exposure), never $3,000,000 (a pre-computed annual total) — the annual projection is computed separately, downstream, from "amount" and "recurrence_basis" together, and must NOT be pre-calculated into "amount" itself. Set "recurrence_basis" to the stated cycle so the downstream system can compute the multi-cycle projection on its own — do not do that multiplication yourself and put the result in "amount". Re-analyzing the identical document must always produce the identical "amount" for the identical finding — if you find yourself multiplying by a shipment count, a number of months, or "annually", stop and use the single-occurrence figure instead.
